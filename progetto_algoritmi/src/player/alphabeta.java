	...
	public MNKCell iterativeDeepening(MNKBoard board, MNKCell[] FC, int maxDepth, Transposition_table TT, killer_heuristic killer,  int distance_from_root, EvaluationTool eval, long startTime, long key){
		...
		while (!outOfTime() && depth < maxDepth) {
			...
			for (int i = 0; i< size; i++) {
				...
				//System.out.println("sto testando, avendo gi‡ generato la chiave, la mossa " + d.i + " " + d.j + " ora faccio chiamata a min " + " con chiave " + key + " con numero random " + TT.getStorage(d.i, d.j, MNKPlayer.ourState));
			...
			//System.out.println("sono nel while e la mia selected cell Ë " + selected_cell.i + " " + selected_cell.j + " con chiave " + key + " e storage " + TT.getStorage(selected_cell.i, selected_cell.j, MNKPlayer.ourState));
			if (outOfTime()) {
				...
				if (!previousEvaluated) {
					if (calculatedMoves < size/2) {
						...
						//System.out.println("sono nel while e la mia selected cell Ë cambiata in  " + selected_cell.i + " " + selected_cell.j + " con chiave " + key + " e storage " + TT.getStorage(selected_cell.i, selected_cell.j, MNKPlayer.ourState));
					}
				}
				else {
		...
		//System.out.println("ho finito il while e la mia selected cell Ë cambiata in  " + selected_cell.i + " " + selected_cell.j + " con chiave " + key + " e storage " + TT.getStorage(selected_cell.i, selected_cell.j, MNKPlayer.ourState));
		/*for(int i=0; i<size; i++) {
			if(FC[i].i != selected_cell.i || FC[i].j != selected_cell.j) {
				System.out.println("la mossa dell'avversario Ë " + FC[i].i + " " + FC[i].j + " con numero random " + TT.getStorage(FC[i].i, FC[i].j, MNKPlayer.enemyState));
				long mykey = TT.generate_key(key, selected_cell.i, selected_cell.j, MNKPlayer.ourState);
				System.out.println(" la mia chiave generata Ë " + mykey);
				System.out.println("la chiave che genererebbe Ë " + TT.generate_key(mykey, FC[i].i, FC[i].j, MNKPlayer.enemyState));
				}
		}*/
		//System.out.println("Sono arrivato fino a depth: " + depth);
	...
	*/
	...
	protected int min(MNKBoard board, int alpha, int beta, int depth, int distance_from_root, EvaluationTool eval, boolean saveNode, long key) {
		...
		for (int i = 0; i< lenght; i++) {
			...
			//System.out.println("sono in min e la chiave precedente Ë " + key);
